...
if CENARIO == 1:
    ...
    NOME_CENARIO    = "🚀 BULL RUN 2021 (ALVO: $19K+)"
elif CENARIO == 2:
    DATA_INICIO_STR = "2022-01-01"
    DATA_FIM_STR    = "2022-12-31"
    NOME_CENARIO    = "🛡️ BEAR 2022 (ALVO: FICAR LÍQUIDO)"
else:
    DATA_INICIO_STR = "2025-01-01"
    DATA_FIM_STR    = "2026-02-16"
    NOME_CENARIO    = "🛡️ MARKET 2025/26 (ALVO: NÃO QUEBRAR)"

# --- CONFIGURAÇÕES V141 (DISCIPLINED GOD - V136 REFINED) ---
BANCA_INICIAL = 60.00
...
# PARÂMETROS TÉCNICOS
EMA_FAST        = 200   
EMA_MACRO       = 800   # O JUIZ (ABAIXO DISSO = CASH)
ADX_MIN         = 30    # Tendência Forte
ADX_NUCLEAR     = 50    # GATILHO 30X
CHOP_NUCLEAR    = 40.0  # Só ativa 30x se o mercado estiver MUITO limpo
ATR_LEN         = 14
ST_MULTIPLIER   = 3.0   

# GESTÃO DINÂMICA
ALAVANCAGEM_BASE    = 5     
ALAVANCAGEM_NUCLEAR = 30    # A ARMA SECRETA
RISCO_BASE          = 0.05  
RISCO_MAX           = 0.15  
BOOST_STEP          = 0.025 
...
def calcular_indicadores_nativos(df):
    ...
    range_hl = (hh - ll).replace(0, 0.00001)
    ...
    st_lower = [0.0] * len(df); st_upper = [0.0] * len(df); st_trend = [1] * len(df)
    close_vals = close.values
    basic_upper = df['st_basic_upper'].values; basic_lower = df['st_basic_lower'].values
...
def run_backtest_v141():
    print(f"🎯 INICIANDO V141 DISCIPLINED GOD (V136 REFINED)...")
    ...
    for ts in timeline:
        ...
        for symbol in list(posicoes_abertas.keys()):
            ...
            # 3. SEGURANÇA MACRO (NOVO)
            # Se o preço cair abaixo da EMA 800, fecha tudo na hora! (Evita ficar preso no inverno)
            elif candle['close'] < candle['ema_macro']:
                 fechou = True; motivo = "SAÍDA DE EMERGÊNCIA (MACRO BEAR)"; p_saida = candle['close']

            # 4. PIRAMIDAGEM
            elif not fechou and pos['adds'] < MAX_PYRAMID:
                if candle['adx'] > ADX_MIN:
                    trigger = candle['atr'] * PYRAMID_STEP
                    if (candle['close'] - pos['ultima_entrada']) > trigger:
                        
                        # NUCLEAR LOGIC: Só usa 30x na adição se ADX > 50 E CHOP < 40
                        nuclear_ready = (candle['adx'] > ADX_NUCLEAR and candle['chop'] < CHOP_NUCLEAR)
                        lev_add = ALAVANCAGEM_NUCLEAR if nuclear_ready else ALAVANCAGEM_BASE
                        ...
                        if (banca_atual - pos['margem_total'] - margem_extra) > 0:
                            ...
                            notional_ant = pos['margem_total'] * lev_atual
                            notional_nov = margem_extra * lev_add
                            pos['preco_medio'] = ((notional_ant * pos['preco_medio']) + (notional_nov * novo_preco)) / (notional_ant + notional_nov)
        ...
        # B. ENTRADAS V141 (LONG ONLY + FILTRO DE FERRO)
        if len(posicoes_abertas) < MAX_TRADES_SIMULTANEOS and banca_atual > 10:
            for symbol in COINS:
                ...
                # 1. O PORTEIRO: PREÇO > EMA 800 (SEMPRE)
                if candle['close'] > candle['ema_macro']:
                    # 2. ADX FORTE
                    if candle['adx'] > ADX_MIN:
                        # 3. TENDÊNCIA LOCAL
                ...
                if sinal:
                    ...
                    # ALAVANCAGEM NUCLEAR CONDICIONAL
                    # Só 30x se for MUITO LIMPO (ADX > 50 e Chop < 40)
                    nuclear_ready = (candle['adx'] > ADX_NUCLEAR and candle['chop'] < CHOP_NUCLEAR)
                    lev_inicial = ALAVANCAGEM_NUCLEAR if nuclear_ready else ALAVANCAGEM_BASE
    ...
    print("\n" + "="*60)
    print(f"🎯 RESULTADO V141 (DISCIPLINED GOD)")
    print("="*60)
    ...
    if total > 0:
        ...
        print("🏆 Trades Disciplinados:")
        for t in tops:
            print(f"   {t['symbol']} | +${t['lucro']:.2f}")

if __name__ == "__main__":
    try: run_backtest_v141()
